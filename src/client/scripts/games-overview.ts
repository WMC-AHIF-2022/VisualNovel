...
async function init() {
  await fetchRestEndpoint(
    "http://localhost:63342/VisualNovel/src/client/html/games.html",
    "GET"
  );
  let gameinfo = getAllGameInfos();
  let list = document.getElementById("gameOverview");
  let html =
    '<li><h4 id="' + '" class="clickable">`${gameinfo.getGameName()}` ';
  html +=
    "</h4><p>`${gameinfo.getDescription()} ${gameinfo.getCreator()}`</p></li>";
  list.innerHTML += html;
